...
void *bind_it(char *kernel, char *s ,char *version)
{
    char buffer [1024];
    void *fun = NULL;
...
static void bind_functions (void)
{
  ...
  the_compute = bind_it(kernel,"compute",version);
  ...
  the_init =  bind_it(kernel,"init",version);
  the_draw =  bind_it(kernel,"draw",draw_param);
  the_finalize =  bind_it(kernel,"finalize",version);
  the_refresh_img = bind_it(kernel,"refresh_img",version);
  
  if (!opencl_used) {
    the_first_touch =  bind_it(kernel,"ft",version);
  }
